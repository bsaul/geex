...
```{r point_estimates, echo = TRUE}

psi_i <- lapply(split_data, function(dt) {
  SB_ex2_eefun(dt)
})

# In theory I can solve the following function (without the abs()) to get formulas
# for theta. 

psi <- function(theta){
  psi_ <- lapply(psi_i, function(f) f(theta))
  abs(sum(unlist(psi_)))
}

# Can I solve the equations numerically instead? This seems hard. 
# How do I know where to start?  Here I start at the truth. 

coef(optimx::optimx(par = c(5, 4, 5/4), fn = psi))

# Notice that the optimization esitmates are pretty bad compared to the closed form
# estimators
