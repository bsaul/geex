...
n <- 50
beta <- c(0.5, 2)
dt <- data.frame(X  = cbind(1, rep(0:1, each = n/2)),
                 e  = rnorm(n),
                 Y  = X %*% beta + e,
                 id = 1:n)
...
SB_ex8_eefun(split_data[[2]])(c(.5, 2))
...
example <- list(eeFUN   = SB_ex9_eefun, 
  ...
  psi1 <- lapply(split_data, function(data_i){
    SB_ex9_eefun(data = data_i)
  })

numDeriv::jacobian(psi1[[1]], x = c(0, 0, 0, 0))
  psi1[[1]](c(1, 0, 0, 0))
  psi <- function(theta){
