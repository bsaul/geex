...
Example 4 calculates an instumental variable estimator. The variables ($Y_3, W^1, Z^1$) are observed according to:
...
\psi_k(Y^1_i, \theta) = 
\begin{cases}
(Y_{2i} - \theta)  & \text{when } |(Y_{1i} - \theta)| \leq k \\
...
Example 8 demonstrates robust regression for estimating $\beta$ from 100 observations generated from $Y_4 = 0.1 + 0.1 X^1_i + 0.5 X^2_i + \epsilon_i$, where $\epsilon_i \sim N(0, 1)$, $X_1$ is defined as above, and the first half of the observation have $X_{2i} = 1$ and the others have $X_{2i} = 0$. 
