...
By default, `geex` uses the [`rootSolve::multiroot`](https://cran.r-project.org/web/packages/rootSolve/rootSolve.pdf) function for finding roots of a set of estimating equations when `compute_roots = TRUE` in `m_estimate()`. However, a user can choose a different root find find algorithm via the `root_control` argument.
...
multiroot_results <- m_estimate(
  estFUN = myefun, 
  data  = geexex,
  root_control = setup_rootFUN(start = 3))
```
...
uniroot_results <- m_estimate(
  estFUN = myefun, 
  data  = geexex,
  root_control = setup_rootFUN(rootFUN = stats::uniroot, interval = c(0, 10)))
```
